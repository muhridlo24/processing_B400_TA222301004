...
def preprocessing():
	...
	for key,value in data_processing[current_processing]['performance_log'].items():
		if(re.match("rpm",key)):
			...
			print("{} selesai dengan waktu eksekusi: {}".format(key,time.time()-ppp))
	...
	latensi_2.append(datetime.now()-temp_time)
	# print(result)
	print("Ended {} at: {}".format(data_processing[current_processing]['machine_id'],datetime.now()-temp_time))
	temp_time=datetime.now()
	return
...
def cloud_run():
	...
	print("waktu pengiriman: ",datetime.now())
	...
	print("elapsed time: ",resp.elapsed.total_seconds())

	latensi_3.append(datetime.now()-pppp)
	print(datetime.now()-pppp)
	print("Respond: ",resp)
...
if __name__=='__main__':
	...
	while 1:
			...
			data={
				"LHL01":{
					...
					"performance_log":{
						...
						"vibration":','.join(map(str,np.random.uniform(-0.005,0.001,200)))
					}
				}
			}
			line="LHL"
		...
		if(receive==1):
			...
			latensi.append(datetime.now()-tm_time)
			# to_be_saved=[]
			...
			for i in (key_machine):
				if(total_data[i]['timedelta']>=(get_timedelta(deviceName=i)-1)):
					...
					total_data.pop(i)
					print("Average execution time: ",np.mean(latensi))
					print("total data bangh: ",total_data)
					t_processing=threading.Thread(target=preprocessing)
					t_processing.run()

			if((datetime.now()-time_reference).seconds>=59):
				print("Average penerimaaan data: ",np.mean(latensi))
				print("Average waktu eksekusi preprocessing: ",np.mean(latensi_2))
				print("Average latensi pengiriman data: ",np.mean(latensi_3))
				latensi_2=[]
				latensi=[]
				latensi_3=[]
				time_reference=datetime.now()
