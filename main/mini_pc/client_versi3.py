...
def stability(rpm_data):
	...
	return mean_rpm,std_dev_rpm,cv
...
def preprocessing():
	...
	for key,value in data_processing[current_processing]['performance_log'].items():
		temp_time=datetime.now()
		if(re.match("rpm",key)):
			rpm_mean,rpm_std,rpm_cv=stability(data_processing[current_processing]['performance_log'][key])
			...
			print("memori rpm: ",getsizeof(result[key]),result[key].__sizeof__())
			...
			print("{} selesai dengan waktu eksekusi: {}".format(key,datetime.now()-temp_time))
			print("memori vibrasi: ",getsizeof(result[key]),result[key].__sizeof__())
			...
			print(result[key])
			print("{} selesai dengan waktu eksekusi: {}".format(key,datetime.now()-temp_time))
			print("memori temperatur: ",getsizeof(result[key]),result[key].__sizeof__())

	result['time']=datetime.now().isoformat("@","minutes")
	result['machine_id']=data_processing[current_processing]['machine_id']
	result['line']=data_processing[current_processing]['line']
	result['plant']=data_processing[current_processing]['plant']
	result['zona']=data_processing[current_processing]['zona']

	print("memori result: ",getsizeof(result),result.__sizeof__())
		...
		if(check_internet_connection() is True):
			...
			if(len(reader)>0):
				...
				for i in range(len(reader)):
					...
					# resp = requests.post("http://127.0.0.1:5000/warning-machine",json=reader[0])
...
if __name__=='__main__':
	...
	# 	'''untuk testing tanpa raspi'''
		if((datetime.now()-time_reference_3).seconds>=1):
			zzz=np.random.uniform(0,1)
			data={
				"LHL01":{
					"timestamp":datetime.now().isoformat("@","seconds"),
					"performance_log":{
						"temperature":np.random.uniform(40,50),
						"rpm":[2500,np.random.uniform(2450,2550)],
						"vibration":','.join(map(str,np.random.uniform(-zzz,zzz,200)))
					},
					"line":"LHL",
					"zona":"Zona A",
					"plant":"J2"
		...
		if(receive==1):
			...
			line=data[list(data.keys())[0]]['line']
			zona=data[list(data.keys())[0]]['zona']
			plant=data[list(data.keys())[0]]['plant']
			# data=json.loads(data)
			...
			with open('total_data.json','r+') as file:
				total_data = json.load(file)
				# # print(new_data.values())
				# file_data[list(data.keys())[0]]=data[list(data.keys())[0]]

				with open('rekap.json','r+') as file_1:
					rekap=json.load(file_1)

				# # python object to be appended
				while True:
					if(list(data.keys())[0] not in list(total_data.keys())):
						if(list(data.keys())[0] not in list(rekap.keys())):
							rekap[list(data.keys())[0]]=copy.deepcopy(data[list(data.keys())[0]])
							rekap[list(data.keys())[0]]['machine_id']=list(data.keys())[0]
							rekap[list(data.keys())[0]].pop('timestamp')
							
							for i,value in (rekap[list(data.keys())[0]]['performance_log'].items()):
								rekap[list(data.keys())[0]]['performance_log'][i]=[value]
						...
						total_data[list(data.keys())[0]]['time_reference']=datetime.now().strftime("%Y-%m-%d %H:%M:%S")
						total_data[list(data.keys())[0]]['timedelta']=(datetime.now()-datetime.strptime(total_data[list(data.keys())[0]]['time_reference'],"%Y-%m-%d %H:%M:%S")).seconds
						total_data[list(data.keys())[0]]['length']=0
						break

					else:
						total_data[list(data.keys())[0]]['timedelta']=(datetime.now()-datetime.strptime(total_data[list(data.keys())[0]]['time_reference'],"%Y-%m-%d %H:%M:%S")).seconds
						total_data[list(data.keys())[0]]['length']+=1
						if(total_data[list(data.keys())[0]]['timedelta']>(get_timedelta(deviceName=list(data.keys())[0])-1)
	 						or (total_data[list(data.keys())[0]]['length']>get_timedelta(deviceName=list(data.keys())[0]))):
							current_processing=list(data.keys())[0]
							data_processing[list(data.keys())[0]]=copy.deepcopy(total_data[list(data.keys())[0]])
							print(total_data.keys())
							total_data.pop(list(data.keys())[0])
							t_processing=threading.Thread(target=preprocessing)
							t_processing.run()
						else:
							for key, value in data[list(data.keys())[0]]['performance_log'].items():
								if (key in rekap[list(data.keys())[0]]['performance_log']):
									rekap[list(data.keys())[0]]['performance_log'][key].append(value)
				...
				"""Save To Local File"""
				print("Time delta: ",total_data[list(data.keys())[0]]['timedelta'])
				...
				with open('rekap.json','w') as file_1:
					json.dump(rekap,file_1)
					file_1.close()
